...
const logger = new Logger('LPZ mediasoup Worker');
...
class Worker extends EnhancedEventEmitter
{
	...
	{
		logger.debug('constructor() | [logLevel:%o, logTags:%o, rtcMinPort:%o, rtcMaxPort:%o, dtlsCertificateFile:%o, dtlsPrivateKeyFile:%o]',
			logLevel, logTags, rtcMinPort, rtcMaxPort, dtlsCertificateFile, dtlsPrivateKeyFile);
		...
		this._workerLogger = new Logger(`LPZ mediasoup worker[pid:${this._child.pid}]`);
	...
	async dump()
	{
		const dump = this._channel.request('worker.dump');
   ...
   *
	...
	{
		logger.debug('updateSettings() | [logLevel:%o, logTags:%o]', logLevel, logTags);
   ...
   *
	...
	{
		logger.debug('createRouter() | [mediaCodecs:%o]', mediaCodecs);
		...
		logger.debug('createRouter() | new Router [internal:%o, data:%o]', internal, data);
		...
		logger.debug('createRouter() | [router:%o]', router);
