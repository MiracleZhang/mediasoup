...
const logger = new Logger('LPZ mediasoup Router');
...
class Router extends EnhancedEventEmitter
{
	...
	constructor({ internal, data, channel })
	{
		...
		logger.debug('constructor() | [internal:%o, data:%o, channel:%o]', internal, data, channel);
	...
	async dump()
	{
		const dump = this._channel.request('router.dump', this._internal);
		logger.debug('dump() | [dump:%o]', dump);
		return dump;
	}
   ...
   *
	...
	{
		logger.debug('createWebRtcTransport() | [listenIps:%o, enableUdp:%o, enableTcp:%o, preferUdp:%o, preferTcp:%o, initialAvailableOutgoingBitrate:%o, minimumAvailableOutgoingBitrate:%o, appData:%o]',
			listenIps, enableUdp, enableTcp, preferUdp, preferTcp, initialAvailableOutgoingBitrate, minimumAvailableOutgoingBitrate, appData);
		...
		logger.debug('createWebRtcTransport() | new WebRtcTransport [data:%o, internal:%o]', data, internal);
		...
		logger.debug('createWebRtcTransport() | [transport:%o]', transport);
   ...
   *
	...
	{
		logger.debug('createPlainRtpTransport() | [listenIp:%o, rtcpMux:%o, comedia:%o, multiSource:%o, appData:%o]',
			listenIp, rtcpMux, comedia, multiSource, appData);
		...
		logger.debug('createPlainRtpTransport() | new PlainRtpTransport [data:%o, internal:%o]', data, internal);
		...
		logger.debug('createPlainRtpTransport() | [transport:%o]', transport);
		this._transports.set(transport.id, transport);
		transport.on('@close', () => this._transports.delete(transport.id));
		transport.on('@newproducer', (producer) => this._producers.set(producer.id, producer));
		transport.on('@producerclose', (producer) => this._producers.delete(producer.id));
   ...
   *
	...
	{
		logger.debug('createPipeTransport() | [listenIp:%o, appData:%o]', listenIp, appData);
		...
		logger.debug('createPipeTransport() | new PipeTransport [data:%o, internal:%o]', data, internal);
		...
		logger.debug('createPipeTransport() | [transport:%o]', transport);
		this._transports.set(transport.id, transport);
		transport.on('@close', () => this._transports.delete(transport.id));
		transport.on('@newproducer', (producer) => this._producers.set(producer.id, producer));
		transport.on('@producerclose', (producer) => this._producers.delete(producer.id));
	...
	{
		logger.debug('pipeToRouter() | [producerId:%o, router:%o, listenIp:%o]', producerId, router, listenIp);
		if (!producerId)
		...
		logger.debug('pipeToRouter() | [producer:%o]', producer);
		if (!producer)
		...
		logger.debug('pipeToRouter() | [pipeTransportPair:%o]', pipeTransportPair);
		if (pipeTransportPair)
		{
			localPipeTransport = pipeTransportPair[0];
			remotePipeTransport = pipeTransportPair[1];
			logger.debug('pipeToRouter() | 1 [localPipeTransport:%o, remotePipeTransport:%o]', localPipeTransport, remotePipeTransport);
		}
		else
		{
			try
			{
				...
				logger.debug('pipeToRouter() | 2 [localPipeTransport:%o, remotePipeTransport:%o]', localPipeTransport, remotePipeTransport);
				await Promise.all(
					[
		...
		try
		{
			...
			logger.debug('pipeToRouter() | localPipeTransport.consume [pipeConsumer:%o]', pipeConsumer);
			...
			logger.debug('pipeToRouter() | remotePipeTransport.produce [pipeProducer:%o]', pipeProducer);
	...
	{
		logger.debug('createAudioLevelObserver() | [maxEntries:%o, threshold:%o, interval:%o]', maxEntries, threshold, interval);

		const internal = { ...this._internal, rtpObserverId: uuidv4() };
		const reqData = { maxEntries, threshold, interval };

		logger.debug('createAudioLevelObserver() | [reqData:%o, internal:%o]', reqData, internal);
		...
		logger.debug('createAudioLevelObserver() | new AudioLevelObserver [audioLevelObserver:%o]', audioLevelObserver);
	...
	{
		logger.debug('canConsume() | [producerId:%o, rtpCapabilities:%o]', producerId, rtpCapabilities);
		const producer = this._producers.get(producerId);

		logger.debug('canConsume() | [producer:%o]', producer);
		if (!producer)
		{
			logger.error(
