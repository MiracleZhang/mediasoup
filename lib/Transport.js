...
const logger = new Logger('LPZ mediasoup Transport');
...
class Transport extends EnhancedEventEmitter
{
		...
		})
	{
		...
		logger.debug('constructor() | [internal:%o, channel:%o, appData:%o, getRouterRtpCapabilities:%o, getProducerById:%o]',
			internal, channel, appData, getRouterRtpCapabilities, getProducerById);
	...
	async dump()
	{
		const dump = this._channel.request('transport.dump', this._internal);
	...
	async getStats()
	{
		const state = this._channel.request('transport.getStats', this._internal);
		logger.debug('getStats() | [state:%o]', state);
		return state;
	}
   ...
   *
	...
	{
		logger.debug('produce() | [id:%o, kind:%o, rtpParameters:%o, paused:%o, appData:%o]',
			id, kind, rtpParameters, paused, appData);
		...
		logger.debug('produce() | [status:%o]', status);
		const data =
		{
			kind,
			rtpParameters,
		...
		logger.debug('produce() | new Producer [internal:%o, data:%o]', internal, data);
		...
		logger.debug('produce() | [producer:%o]', producer);
   ...
   *
	...
	{
		logger.debug('consume() | [producerId:%o, rtpCapabilities:%o, preferredLayers:%o, paused:%o, appData:%o]',
			producerId, rtpCapabilities, preferredLayers, paused, appData);
		...
		logger.debug('consume() | [producer:%o]', producer);
		...
		logger.debug('consume() | new Consumer [internal:%o, data:%o, status:%o]', internal, data, status);
