...
const logger = new Logger('LPZ mediasoup Consumer');
...
class Consumer extends EnhancedEventEmitter
{
	...
	constructor({ internal, data, channel, appData, paused, producerPaused, score })
	{
		...
		logger.debug('constructor() | [internal:%o, data:%o, channel:%o, appData:%o, paused:%o, producerPaused:%o, score:%o]',
			internal, data, channel, appData, paused, producerPaused, score);
	...
	async dump()
	{
		const dump = this._channel.request('consumer.dump', this._internal);
	...
	async getStats()
	{
		const state = this._channel.request('consumer.getStats', this._internal);
	...
	{
		logger.debug('setPreferredLayers() | [spatialLayer:%o, temporalLayer:%o]', spatialLayer, temporalLayer);
